...
unique_host_id, offset, count = np.unique(sorted_host_id, return_index=True, return_counts=True)
for o, c, hid in zip(offset, count, unique_host_id):
    assert np.all(sorted_rank[o:o+c]==np.arange(c, dtype=int)) # ranks should be sequential
