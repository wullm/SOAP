...
def read_vr_group_sizes(basename, suffix, comm):
    ...
    if comm_rank == 0:
        ...
        while file_nr < nr_files:
            ...
            with h5py.File(filename, "r") as infile:
                nr_bound_part_in_file.append(int(infile["Num_of_particles_in_groups"][...]))
            ...
            with h5py.File(filename, "r") as infile:
                nr_unbound_part_in_file.append(int(infile["Num_of_particles_in_groups"][...]))
    ...
    (nr_bound_part_in_file, nr_unbound_part_in_file, nr_groups_in_file, nr_files) = \
        comm.bcast((nr_bound_part_in_file, nr_unbound_part_in_file, nr_groups_in_file, nr_files))

    # Read offsets from the catalog_groups files
    mf = phdf5.MultiFile(vr_filename("catalog_groups"), file_nr_dataset="Num_of_files")
    offset_bound, offset_unbound, group_size = mf.read(("Offset", "Offset_unbound", "Group_Size"), unpack=True)
    ...
    last_in_file = np.cumsum(nr_groups_in_file) - 1 # Global index of last halo in each file
    local_offset = comm.scan(nr_halos_local) - nr_halos_local # Nr. of halos on previous MPI ranks
    for file_nr in range(nr_files):
        if nr_groups_in_file[file_nr] > 0:
            ...
            if local_index >= 0 and local_index < nr_halos_local:
                ...
                nr_parts_bound[local_index] = nr_bound_part_in_file[file_nr] - offset_bound[local_index]
                nr_parts_unbound[local_index] = nr_unbound_part_in_file[file_nr] - offset_unbound[local_index]
    ...
    for rank in range(comm_rank+1, comm_size):
        assert (first_offset_bound[rank] is None) == (first_offset_unbound[rank] is None)
    ...
    for halo_nr in range(nr_halos_local-1):
        if not(is_last[halo_nr]):            
            ...
            nr_parts_unbound[halo_nr] = offset_unbound[halo_nr+1] - offset_unbound[halo_nr]
    ...
    if nr_wrong > 0:
        index_wrong = np.arange(nr_halos_local, dtype=int)[nr_parts_bound+nr_parts_unbound != group_size]
        for rank in range(comm_size):
            if comm_rank == rank:
                print(f"Rank {comm_rank}:")
                for i in index_wrong:
                    print(f"  Halo {i} of {nr_halos_local}: {nr_parts_bound[i]} + {nr_parts_unbound[i]} != {group_size[i]}")
            comm.barrier()
        raise RuntimeError(f"Number of particles in halos has been computed wrongly for {nr_wrong} halos!")
